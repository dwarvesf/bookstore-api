...
func TestHandler_GetBooks(t *testing.T) {
	...
	for name, tt := range tests {
		...
		if tt.mocked.pageSize != "" {
			query.Add("pageSize", tt.mocked.pageSize)
		}
		if tt.mocked.sort != "" {
		...
		if tt.mocked.expGetBooks {
			ctrlMock.EXPECT().GetBooks(mock.Anything, mock.Anything, mock.Anything).Return(tt.mocked.books, tt.mocked.getBooksErr)
		}
